...
export class TextStyles {
  ...
  private static getPlatformPrefix(): string {
    ...
    if (process.env['NODE_ENV'] === 'test' || platformValue === 'unknown') {
      return 'orva-ui-h5-text';
    }
    return `orva-ui-${platformValue}-text`;
  }
  ...
  static getStyle(props: Partial<TextProps>): React.CSSProperties {
    ...
    // 处理文本装饰
    const textDecorationValue: string[] = [];
    ...
    return {
      ...
      color: colorStyles,
      textAlign: align,
      textDecoration: finalDecoration as React.CSSProperties['textDecoration'],
