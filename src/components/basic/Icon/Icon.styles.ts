...
export class IconStyles {
  ...
  private static getPlatformPrefix(): string {
    ...
    return `orva-ui-${platformValue}-icon`;
  }
  ...
  static readonly STATUS_MAP: Record<IconStatus, { opacity: number; cursor: string; pointerEvents: string }> = {
    default: { opacity: 1, cursor: 'default', pointerEvents: 'auto' },
    success: { opacity: 1, cursor: 'default', pointerEvents: 'auto' },
    warning: { opacity: 1, cursor: 'default', pointerEvents: 'auto' },
    error: { opacity: 1, cursor: 'default', pointerEvents: 'auto' },
    info: { opacity: 1, cursor: 'default', pointerEvents: 'auto' },
  ...
  static readonly THEME_STYLES: Record<IconTheme, { strokeWidth?: number; fill?: string; stroke?: string }> = {
    ...
    rounded: { strokeWidth: 1.5, fill: 'none', stroke: 'currentColor' },
    sharp: { strokeWidth: 1.5, fill: 'none', stroke: 'currentColor' },
  };
  ...
  static getAnimationClassName(animation: string): string {
    const animationMap: Record<string, string> = {
      spin: 'orva-ui-icon--spin',
      pulse: 'orva-ui-icon--pulse',
      bounce: 'orva-ui-icon--bounce',
      shake: 'orva-ui-icon--shake',
    };
