...
export function useFormLogic(props: UseFormLogicProps) {
  ...
  const size = rawSize as FormSize;
  ...
  const validateField = useCallback(
    async (name: string): Promise<{ valid: boolean; errors: string[]; value: any }> => {
      ...
      try {
        ...
        // 验证必填
        if (fieldRules.some((rule: FormRule) => rule.required)) {
          if (field.value === undefined || field.value === null || field.value === '') {
            const requiredRule = fieldRules.find((rule: FormRule) => rule.required);
        ...
        if (typeof field.value === 'string') {
          fieldRules.forEach((rule: FormRule) => {
