...
export class FormStyles {
  ...
  static getClassName(props: Partial<FormProps>): string {
    ...
    const classes = ['orva-ui-form', `orva-ui-form--${layout}`, `orva-ui-form--${size}`, className].filter(Boolean);
  ...
  static getItemClassName(props: {
    ...
    const classes = [
      'orva-ui-form-item',
      `orva-ui-form-item--${layout}`,
      `orva-ui-form-item--${size}`,
      hasError && 'orva-ui-form-item--error',
      className,
    ].filter(Boolean);
