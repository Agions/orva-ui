...
export class TimePickerStyles {
  ...
  static getClassName(config: {
    ...
    const classes = [
      'orva-ui-timepicker',
      `orva-ui-timepicker--${size}`,
      `orva-ui-timepicker--${variant}`,
      `orva-ui-timepicker--${status}`,
    ];

    if (disabled) classes.push('orva-ui-timepicker--disabled');
    if (readonly) classes.push('orva-ui-timepicker--readonly');
    if (loading) classes.push('orva-ui-timepicker--loading');
