...
export class CheckboxStyles {
  ...
  private static getPlatformPrefix(): string {
    return 'orva-ui-checkbox';
  }

  /** 尺寸映射 */
  static readonly SIZE_MAP: Partial<Record<
    CheckboxSize,
  ...
  /** 状态颜色映射 */
  static readonly STATUS_COLORS: Partial<Record<
    CheckboxStatus,
    {
  ...
  /** 颜色主题映射 */
  static readonly COLOR_THEMES: Partial<Record<
    CheckboxColor,
  ...
  static getStyle(props: Partial<CheckboxProps>): React.CSSProperties {
    ...
    const sizeStyles = this.SIZE_MAP[size] as { fontSize: number; size: number; borderRadius: number; padding: number; iconSize: number } | undefined ?? { fontSize: 28, size: 24, borderRadius: 5, padding: 8, iconSize: 16 };
    const statusStyles = this.STATUS_COLORS[status] as { backgroundColor: string; borderColor: string; checkColor: string; textColor: string; icon: string } | undefined ?? { backgroundColor: '#ffffff', borderColor: '#d1d5db', checkColor: '#0ea5e9', textColor: '#374151', icon: '✓' };
    const variantStyles = this.VARIANT_STYLES[variant];
    const colorTheme = this.COLOR_THEMES[color] as { primary: string; secondary: string; background: string; border: string } | undefined ?? { primary: '#0ea5e9', secondary: '#e0f2fe', background: '#ffffff', border: '#0ea5e9' };
  ...
  static getLabelStyle(props: {
    ...
    const sizeStyles = (this.SIZE_MAP[size] ?? { fontSize: 28, size: 24, borderRadius: 5, padding: 8, iconSize: 16 }) as { fontSize: number; size: number; borderRadius: number; padding: number; iconSize: number };
  ...
  static getHelperTextStyle(props: {
    ...
    const sizeStyles = (this.SIZE_MAP[size] ?? { fontSize: 28, size: 24, borderRadius: 5, padding: 8, iconSize: 16 }) as { fontSize: number; size: number; borderRadius: number; padding: number; iconSize: number };
    const statusStyles = (this.STATUS_COLORS[status] ?? { backgroundColor: '#ffffff', borderColor: '#d1d5db', checkColor: '#0ea5e9', textColor: '#374151', icon: '✓' }) as { backgroundColor: string; borderColor: string; checkColor: string; textColor: string; icon: string };
