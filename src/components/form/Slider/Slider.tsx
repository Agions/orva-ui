...
export const Slider = createComponent<SliderProps, SliderRef>({
  ...
  defaultProps: {
    ...
    reverse: false,
    tooltip: undefined,
    size: 'small',  // 使用 SliderSize 类型定义
    variant: 'primary',  // 使用 SliderVariant 类型定义
    className: '',
    style: undefined,
    onChange: undefined,
  ...
  render: (props, ref) => {
    ...
    const { state: _interactionState } = useInteractionState({  // unused  // handlers removed
      disabledPress: disabled,
      disabledHover: disabled,
    });

    // const animation = useMicroAnimation({ type: 'button', enabled: !disabled });  // unused
    ...
    return (
      ...
      >
        ...
        {marks && (
          <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0 }}>{renderMarks()}</View>
        )}
      </View>
    );
  },
