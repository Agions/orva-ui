...
export class CascaderStyles {
  ...
  static getClassName(config: {
    ...
    const classes = [
      'orva-ui-cascader',
      `orva-ui-cascader--${size}`,
      `orva-ui-cascader--${variant}`,
      `orva-ui-cascader--${status}`,
    ];

    if (disabled) classes.push('orva-ui-cascader--disabled');
    if (readonly) classes.push('orva-ui-cascader--readonly');
    if (loading) classes.push('orva-ui-cascader--loading');
