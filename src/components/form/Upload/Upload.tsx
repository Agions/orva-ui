  ...
  render: (props, ref) => {
    ...
    const handleFileChange = useCallback(
      (newFileList: UploadFile[]) => {
        ...
        onChange?.(newFileList[0], newFileList);
      },
    ...
    const validateFile = useCallback(
      (file: TaroTempFile): { valid: boolean; error?: string } => {
        const filename = file.name || file.path.split('/').pop() || '';
        const fileSize = (file.size ?? 0) || 0;
          ...
          {fileList.map((file) => (
            <View key={file.uid} style={fileItemStyle}>
              <View style={fileInfoStyle}>
                ...
                <Text style={{ fontSize: 12, color: '#6b7280' }}>{((file.size ?? 0) / 1024).toFixed(2)} KB</Text>
                {file.status === 'uploading' && (
                  <Text style={{ fontSize: 12, color: '#3b82f6' }}>{file.percent}%</Text>
                )}
