...
export class DatePickerStyles {
  ...
  static getClassName(props: {
    ...
    const baseClass = 'orva-ui-datepicker';
    const sizeClass = `orva-ui-datepicker--${size}`;
    const variantClass = `orva-ui-datepicker--${variant}`;
    const statusClass = status !== 'normal' ? `orva-ui-datepicker--${status}` : '';
    const disabledClass = disabled || readOnly ? 'orva-ui-datepicker--disabled' : '';
    const openedClass = opened ? 'orva-ui-datepicker--opened' : '';
    const focusedClass = focused ? 'orva-ui-datepicker--focused' : '';
