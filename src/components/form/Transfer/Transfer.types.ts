...
import type { ReactNode, CSSProperties, TouchEvent } from 'react';
...
/** 尺寸类型 */
export type TransferSize = 'small' | 'medium' | 'large' | 'default';
...
/** 渲染位置类型 */
export type TransferRenderPosition = 'source' | 'target';
...
/** Transfer 原生事件属性 */
export interface TransferNativeProps extends FormItemProps {
	/** 点击事件 */
  onClick?: (_event: TouchEvent) => void;
	/** 长按事件 */
  onLongPress?: (_event: TouchEvent) => void;
	/** 触摸开始事件 */
