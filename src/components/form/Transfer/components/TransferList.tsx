...
/** Transfer列表组件 */
export const TransferList = createComponent<TransferListProps>({
  name: 'TransferList',
  ...
  render: (props, _ref) => {
    ...
    const filterDataSource = useCallback(
      (data: TransferDataSource, searchValue: string) => {
        ...
        return data.filter((item: TransferOption) => {
          if (filterOption) {
    ...
    const calculateSelectAllState = useCallback(
      (data: TransferDataSource) => {
        const enabledData = data.filter((item: TransferOption) => !item.disabled);
        const totalCount = enabledData.length;
        const selectedCount = enabledData.filter((item: TransferOption) => selectedKeys.includes(item.key)).length;
    ...
    const renderFooter = useCallback(() => {
      ...
      const leftData = dataSource.filter((item: TransferOption) => !targetKeys.includes(item.key));
      const rightData = dataSource.filter((item: TransferOption) => targetKeys.includes(item.key));
    ...
    return (
      ...
      >
        ...
        >
          {paginatedData.length > 0 ? paginatedData.map((item: TransferOption, index: number) => renderItem(item, index)) : renderEmpty()}
        </View>
