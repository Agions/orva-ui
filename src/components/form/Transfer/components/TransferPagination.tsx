...
/** Transfer分页组件 */
export const TransferPagination = createComponent<TransferPaginationProps>({
  name: 'TransferPagination',
  ...
  render: (props, _ref) => {
    const { pagination, currentPage, total, onPageChange, disabled } = props;
    ...
    const handleQuickJumperChange = useCallback(
      (e: { detail: { value: string } }) => {
        const newPage = parseInt(e.detail.value);
    ...
    const PaginationButton: React.FC<{
      ...
      ariaLabel: string;
    }> = memo(({ onClick, disabled: btnDisabled, children, ariaLabel }: any) => (
      <View
        style={{
          ...(TransferStyles['getPaginationButtonStyle'](btnDisabled) as React.CSSProperties),
          ...(btnDisabled ? {} : (TransferStyles['getPaginationButtonHoverStyle']() as React.CSSProperties)),
        }}
        onClick={onClick}
    ...
    return (
      <View style={TransferStyles['getPaginationStyle']() as React.CSSProperties} role="navigation" aria-label="分页导航">
        <Text style={TransferStyles['getPaginationInfoStyle']() as React.CSSProperties}>
          {simple
        ...
        {!simple && (
          <View style={{ display: 'flex', gap: '4px' }} role="group" aria-label="分页按钮组">
            ...
            {showQuickJumper && (
              <Input
                type="number"
                style={TransferStyles['getPaginationSelectStyle']() as React.CSSProperties}
                value={currentPage.toString()}
                onInput={handleQuickJumperChange as (e: unknown) => void}
