 ...
 * @description 选择器组件，支持单选/多选模式、搜索、清除、前缀/后缀、验证规则、加载状态、自定义下拉样式和可访问性支持。
 *
 * @example
 * ```tsx
 * <Select options={[{ label: '选项A', value: 'a' }]} onChange={handleChange} />
 * <Select mode="multiple" placeholder="请选择多个" allowClear />
 * ```
 */
...
/** 选择器组件 */
export const Select = createComponent<SelectProps, SelectRef>({
  name: 'Select',
  ...
  render: (props, ref) => {
    ...
    // 计算最终状态
    const finalStatus: SelectStatus | 'disabled' | 'loading' = internalDisabled
      ? 'disabled'
      : loading
        ? 'loading'
    ...
    React.useImperativeHandle(
      ...
      () => ({
        ...
        getStatus: () => finalStatus as SelectStatus,
        validate: async () => {
    ...
    // 处理 Picker 变化
    const handlePickerChange = (e: { detail: { value: number | string } }) => {
      const selectedIndex = typeof e.detail?.value === 'number' ? e.detail.value : parseInt(String(e.detail?.value || 0));
    ...
    return (
      <View style={selectStyles['getContainerStyle']({ size, block: props.block, style: containerStyle })}>
        {/* 标签 */}
