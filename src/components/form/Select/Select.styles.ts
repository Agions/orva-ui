...
export class SelectStyles {
  ...
  > = {
    ...
    borderless: { backgroundColor: 'transparent', borderColor: 'transparent', borderWidth: 0 },
  };
  ...
  > = {
    default: { color: '#111827', borderColor: '#e5e7eb' },
  ...
  static getClassName(props: Partial<SelectProps>): string {
    ...
    const classes = [
      ...
      'orva-ui-select',
      `orva-ui-select--${size}`,
      `orva-ui-select--${variant}`,
      `orva-ui-select--${status}`,
      disabled && 'orva-ui-select--disabled',
      readonly && 'orva-ui-select--readonly',
      bordered && 'orva-ui-select--bordered',
    ].filter(Boolean);
