...
export function usePerformanceMonitor(options: PerformanceMonitorOptions = {}) {
  ...
  const completeRenderMonitoring = useCallback(() => {
    ...
    if (config.thresholds?.renderTime && renderTime > config.thresholds.renderTime) {
      logger.warn(`渲染性能警告: 渲染时间 ${renderTime.toFixed(2)}ms 超过阈值 ${config.thresholds.renderTime}ms`);
  ...
  const monitorMemoryUsage = useCallback(() => {
    ...
    try {
      ...
      if (config.thresholds?.memoryUsage && memoryUsage.used > config.thresholds.memoryUsage * 1024 * 1024) {
        logger.warn(`内存使用警告: ${memoryUsage.used / 1024 / 1024}MB 超过阈值 ${config.thresholds.memoryUsage}MB`);
        config.onPerformanceWarning?.({
          ...metrics,
          memoryUsage,
        });
  ...
  // 生成优化建议
  const generateRecommendations = (metrics: PerformanceMetrics, thresholds?: PerformanceThresholds): Array<{ type: string; issue: string; suggestion: string }> => {
