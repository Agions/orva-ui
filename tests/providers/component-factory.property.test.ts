...
describe('Property 2: Theme Switching Consistency', () => {
  // Use a unique persistKey for this test suite to avoid cross-test contamination
  const testPersistKey = `orva-ui-theme-test-${Date.now()}`;
  ...
  it('should reflect theme mode changes in all consuming components', async () => {
    ...
    render(
      React.createElement(
        ThemeProvider,
        { defaultMode: 'light', persistKey: testPersistKey },
  ...
  it('should provide isDark=false for light mode', () => {
    ...
    const isDarkElement = container.querySelector('[data-testid="is-dark"]');
    expect(isDarkElement).not.toBeNull();
    expect(isDarkElement!.textContent).toBe('false');
    unmount();
  });

  it('should provide isDark=true for dark mode', () => {
    const { container, unmount } = render(
      React.createElement(
        ThemeProvider,
        { defaultMode: 'dark', persistKey: testPersistKey },
        React.createElement(ThemeAwareComponent, null),
      ),
    );
    const isDarkElement = container.querySelector('[data-testid="is-dark"]');
    expect(isDarkElement).not.toBeNull();
    expect(isDarkElement!.textContent).toBe('true');
    unmount();
  });
